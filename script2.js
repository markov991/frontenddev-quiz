...
const btnsOpenModalHTML = document
  .getElementById("q_html")
  .getElementsByTagName("button");
const btnsOpenModalCSS = document
  .getElementById("q_css")
  .getElementsByTagName("button");
const btnsOpenModalJS = document
  .getElementById("q_js")
  .getElementsByTagName("button");
...
const submitingAnsw = function () {
  btnSubmit.addEventListener("click", function () {
    if (!modal.classList.contains("hidden")) {
      for (
        let i = 0;
        i < posibleAnswers.getElementsByTagName("input").length;
        i++
      ) {
        if (
          posibleAnswers.getElementsByTagName("input")[i].checked &&
          i === selectedQuestion.corAnsw
        ) {
          score++;
      ...
      percOfCorrAnsw.innerHTML = `${Math.trunc(
        (score / allBtns.length) * 100
      )}%`;
...
const renderquestion = function () {
  posibleAnswers.insertAdjacentHTML(
    "afterbegin",
    `
